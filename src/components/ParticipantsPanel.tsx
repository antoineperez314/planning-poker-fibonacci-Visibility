    ...
    <div className="grid gap-3">
      {participants.map((participant) => (
        <div
          key={participant.id}
          className={clsx(
            "grid grid-cols-[1fr_auto] items-center gap-3 rounded-lg border bg-white p-3 shadow-sm dark:bg-slate-950",
            outliers.has(participant.id) ? "border-rose-200 dark:border-rose-800" : "border-slate-100 dark:border-slate-800",
          )}
        >
          <div className="min-w-0">
            <div className="flex min-w-0 items-center gap-2">
              <span className="truncate font-semibold text-slate-900 dark:text-cyan-50">{participant.name}</span>
